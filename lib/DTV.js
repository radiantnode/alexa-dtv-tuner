...
module.exports = function (config) {
  var module = {
    errorTypes: {
      ...
      DTV_CHANNEL_DOES_NOT_EXIST: 600,
  ...
  function get (path, options)
  {
    return new Promise(function(resolve, reject){
      var requestOpts = _.merge({
        ...
        // Accept our self-signed cert and pass our X-Simm-Auth header
        rejectUnauthorized: false,
        headers: { 'X-Simm-Auth': config.x_simm_auth }
      }, options);
        ...
        .catch(function(data){
          ...
          if(data && data.statusCode && data.error && data.error.status) {
            ...
            if(errorMessage.match(/channel does not exist/i)) {
              statusCode = module.errorTypes.DTV_CHANNEL_DOES_NOT_EXIST;
            } else {
  ...
  {
    var chLookup = channels.getChannelInfoByCallsign(data.callsign);
